...
public final class ApostleBeamSpell {
    ...
    private float yaw;
    public ApostleBeamSpell(Apostle boss) { this.boss = boss; }
    ...
    public void tick(int tick, boolean second, LivingEntity target) {
        if (tick == 1) {
            ...
            for (int i = 0; i < (second ? 4 : 1); i++) {
                ...
                beams.add(beam);
                var warning = StarFantasyVfx.groundRectangleWarningTrackingGround(beam, 40,
                        2, ApostleBeamEntity.LENGTH, 0, 0xFF0000, false, true);
        ...
        if (tick <= 40) return;
        for (ServerPlayer player : boss.m_9236_().m_45976_(ServerPlayer.class, boss.m_20191_().m_82400_(50))) {
    ...
    public static boolean intersects(LivingEntity target, Apostle boss, float yaw) {
        double angle = Math.toRadians(yaw), fx = -Math.sin(angle), fz = Math.cos(angle);
        double dx = target.m_20185_() - boss.m_20185_(), dz = target.m_20189_() - boss.m_20189_();
        double radius = target.m_20205_() / 2D, forward = dx * fx + dz * fz;
        return forward >= -radius && forward <= ApostleBeamEntity.LENGTH + radius
                && Math.abs(-dx * fz + dz * fx) <= 1 + radius
                && target.m_20191_().f_82292_ >= boss.m_20186_() + .63
                && target.m_20191_().f_82289_ <= boss.m_20186_() + 2.07;
    }
    public void clear() {

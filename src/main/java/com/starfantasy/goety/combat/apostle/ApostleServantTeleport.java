...
public final class ApostleServantTeleport {
    private final ApostleServantEntity servant;
    private int cooldown, windup, receivedHits;
    private Vec3 pending;

    public ApostleServantTeleport(ApostleServantEntity servant) { this.servant = servant; }
    ...
    private boolean canAct(LivingEntity target) {
        return servant.m_9236_() instanceof ServerLevel && servant.m_6084_()
                && !servant.isStaying() && !servant.isCasting()
    ...
    public void tick(LivingEntity target) {
        ...
        if (pending != null) {
            if (!Boolean.TRUE.equals(MobsConfig.ApostleDelayedTeleport.get())
                    || ++windup >= (servant.isSecondPhase() ? 20 : 40)) {
    ...
    private boolean finish(ServerLevel level, Vec3 destination) {
        ...
        if (servant.isInNether() && servant.isSecondPhase()) {
            var trap = new com.Polarice3.Goety.common.entities.util.FireBlastTrap(level,old.f_82479_,old.f_82480_+.25,old.f_82481_);
            trap.setOwner(servant); trap.setAreaOfEffect(3); level.m_7967_(trap);
        }
        cooldown = 80;
        return true;
    }

...
public final class ApollyonFireTrapManager {
    private static final String MANAGED_TRAP_TAG = "starfantasy_apollyon_managed_fire_trap";
    ...
    private static final float TRAP_DAMAGE = 30.0F;
    ...
    public static void cast(ApollyonEntity boss, LivingEntity target) {
        ...
        PENDING_RINGS.add(new PendingRing(boss, base, RING_DELAY_TICKS));
    }
    ...
    public static void castPageantTriple(ApollyonEntity boss, LivingEntity target) {
        ...
        Vec3 center = groundCenterAt(boss.m_9236_(), target.m_20185_(),
    ...
    public static List<Vec3> pageantTriplePositions(Vec3 home, Vec3 first) {
        double dx = first.f_82479_ - home.f_82479_;
        double dz = first.f_82481_ - home.f_82481_;
        double sine = Math.sqrt(3.0D) * 0.5D;
        return List.of(first,
                new Vec3(home.f_82479_ - 0.5D * dx - sine * dz, first.f_82480_,
    ...
    private static void detonate(ServerLevel level, PendingTrap trap) {
        ...
        for (LivingEntity target : level.m_45976_(LivingEntity.class, searchBox)) {
            ...
            target.m_6469_(source, boss.scaleOutgoingDamage(TRAP_DAMAGE));
        }
    }
